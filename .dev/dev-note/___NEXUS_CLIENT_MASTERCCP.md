...
**Updated:** 2025-12-26
**Status:** PLANNING
**Companion:** `___NEXUS_MASTERCCP.md` (Vendor-facing, Phase 12 Complete)
...
|-----|------|--------|----------|
| CCP-C1 | Shared schema supports client context | ⏳ VERIFY | - |
...
| CCP-C6 | Demo data includes client scenarios | ⏳ VERIFY | - |
...
| nexus_invoices | ✅ | Has from_tenant_id (vendor), to_tenant_id (client) |
| nexus_payments | ✅ | Has from_tenant_id (client), to_tenant_id (vendor) |
...
WHERE to_tenant_id = 'TC-ALPH0001';  -- We receive invoices

-- Check: Payments where we are the payer (client)
SELECT * FROM nexus_payments
WHERE from_tenant_id = 'TC-ALPH0001';  -- We make payments
...
### PHASE C1: Context Verification ⏳ PENDING
| # | Task | Status |
|---|------|--------|
| C1.1 | Verify existing schema supports client context | ⏳ |
| C1.2 | Verify adapter has client-perspective queries | ⏳ |
| C1.3 | Verify demo data has client test scenarios | ⏳ |
| C1.4 | Document any schema gaps | ⏳ |

**Validation Query:**
```sql
-- From Alpha Corp (TC-ALPH0001) perspective
-- Alpha is a CLIENT of Beta and Gamma

-- My vendors (relationships where I'm client)
SELECT
    r.vendor_id,
    v.name as vendor_name,
    r.status
FROM nexus_tenant_relationships r
JOIN nexus_tenants v ON v.tenant_vendor_id = r.vendor_id
WHERE r.client_id = 'TC-ALPH0001';

-- Cases I'm tracking (cases where I'm the client)
SELECT * FROM nexus_cases WHERE client_id = 'TC-ALPH0001';

-- Invoices I need to pay (invoices TO me)
SELECT * FROM nexus_invoices WHERE to_tenant_id = 'TC-ALPH0001';
```
