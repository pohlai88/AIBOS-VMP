...
async function pass1ExactMatch(soaLine, invoices) {
  for (const invoice of invoices) {
    ...
    const docMatch =
      (soaLine.invoice_number ?? '').toString().trim().toUpperCase() ===
      (invoice.invoice_number ?? '').toString().trim().toUpperCase();
    const amountMatch = parseFloat(soaLine.amount) === parseFloat(invoice.total_amount);
    const currencyMatch =
      (soaLine.currency_code || soaLine.currency || 'USD') === (invoice.currency || 'USD');
    ...
    const dateExactMatch = haveBothDates
      ? new Date(soaLine.invoice_date).toISOString().slice(0, 10) ===
  ...
  for (const invoice of invoices) {
    ...
    if (docMatch && currencyMatch) {
      const amountMatch = isAmountWithinTolerance(
        ...
        1.0, // RM 1.00 absolute tolerance
