...
export function detectPeriodFromFilename(filename, content = '') {
    ...
    const dateRangePattern = /(\d{4}[-/]\d{1,2}[-/]\d{1,2})[\s_-]?(?:to|until|till|-)[\s_-]?(\d{4}[-/]\d{1,2}[-/]\d{1,2})/i;
    ...
    const statementPattern = /statement\s+(?:for|of|period)\s+(jan|january|feb|february|mar|march|apr|april|may|jun|june|jul|july|aug|august|sep|september|oct|october|nov|november|dec|december)[\s_-]?(\d{4})/i;
...
export function formatUserFriendlyError(error, context = {}) {
    ...
    if (error.includes('Missing') || error.includes('missing') || error.includes('required')) {
        const fieldName = field ? field.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase()) : 'a required field';
        return `Line ${row || '?'}: Missing ${fieldName}. Please add this information to the file.`;
    }

    // Document number errors
    if (error.includes('Document Number') || error.includes('doc_no') || error.includes('invoice_number')) {
    ...
    if (friendlyError.includes('parse') || friendlyError.includes('read')) {
        friendlyError += ' Please check that the file format is correct. You can download a template to see the expected format.';
    }
...
export function getPeriodSuggestions() {
    ...
    return [
        ...
        formatPeriod(lastMonth),
        formatPeriod(twoMonthsAgo)
    ];
}
