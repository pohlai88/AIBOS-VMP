...
import { DatabaseError, ClientError, NotFoundError, ValidationError } from '../src/utils/errors.js';
...
describe('Adapter Branch Coverage - Error Handling Paths', () => {
    ...
    describe('updateCaseStatus - Error Branches', () => {
        test('should throw DatabaseError when database update fails with unhandled error', async () => {
            ...
            try {
                await vmpAdapter.updateCaseStatus('00000000-0000-0000-0000-000000000000', status, updatedByUserId);
        ...
        test('should throw ValidationError for missing parameters', async () => {
            await expect(
                vmpAdapter.updateCaseStatus(null, 'resolved', 'user-id')
            ).rejects.toThrow(ValidationError);

            await expect(
                vmpAdapter.updateCaseStatus('case-id', null, 'user-id')
            ).rejects.toThrow(ValidationError);
        });
    });
    ...
    describe('reassignCase - Error Branches', () => {
        ...
        test('should throw ValidationError for invalid ownerTeam', async () => {
            ...
            await expect(
                vmpAdapter.reassignCase(caseId, invalidTeam)
            ).rejects.toThrow(ValidationError);
        });

        test('should throw ValidationError for missing parameters', async () => {
            await expect(
                vmpAdapter.reassignCase(null, 'ap')
            ).rejects.toThrow(ValidationError);

            await expect(
                vmpAdapter.reassignCase('case-id', null)
            ).rejects.toThrow(ValidationError);
        });
    });
    ...
    describe('escalateCase - Error Branches', () => {
        ...
        test('should throw ValidationError for invalid escalationLevel', async () => {
            ...
            await expect(
                vmpAdapter.escalateCase(caseId, 0, escalatedByUserId)
            ).rejects.toThrow(ValidationError);
        });

        test('should throw ValidationError for missing parameters', async () => {
            await expect(
                vmpAdapter.escalateCase(null, 1, 'user-id')
            ).rejects.toThrow(ValidationError);

            await expect(
                vmpAdapter.escalateCase('case-id', null, 'user-id')
            ).rejects.toThrow(ValidationError);

            await expect(
                vmpAdapter.escalateCase('case-id', 1, null)
            ).rejects.toThrow(ValidationError);
        });
    ...
    describe('createChecklistStep - Error Branches', () => {
        ...
        test('should throw ValidationError for missing parameters', async () => {
            await expect(
                vmpAdapter.createChecklistStep(null, 'label')
            ).rejects.toThrow(ValidationError);

            await expect(
                vmpAdapter.createChecklistStep('case-id', null)
            ).rejects.toThrow(ValidationError);
        });
    });
    ...
    describe('createMessage - Error Branches', () => {
        ...
        test('should throw ValidationError for invalid senderType', async () => {
            ...
            await expect(
                vmpAdapter.createMessage(caseId, body, invalidSenderType)
            ).rejects.toThrow(ValidationError);
        });
    ...
    describe('createNotification - Error Branches', () => {
        ...
        test('should throw ValidationError for missing parameters', async () => {
            await expect(
                vmpAdapter.createNotification(null, 'user-id', 'type', 'title')
            ).rejects.toThrow(ValidationError);

            await expect(
                vmpAdapter.createNotification('case-id', null, 'type', 'title')
            ).rejects.toThrow(ValidationError);
