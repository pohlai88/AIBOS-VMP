...
test.describe('Button and Link Navigation: E2E Tests', () => {
  test.describe('Public Routes - Landing Page', () => {
    test('landing page login button navigates to /login', async ({ page }) => {
      ...
      const loginButton = page.locator('a[href="/login"], button:has-text("Sign In"), a:has-text("Sign In"), a:has-text("Get Started")').first();
  ...
  test.describe('Button Actions', () => {
    test('logout button submits form and redirects', async ({ page }) => {
      ...
      // Find logout button
      const logoutButton = page.locator('button[type="submit"]:has-text("Sign Out"), form[action="/logout"] button').first();
    ...
    test('theme toggle button changes theme', async ({ page }) => {
      ...
      const themeButton = page.locator('button.vmp-theme-toggle, button[aria-label*="theme" i]').first();
