...
test.describe('Days 5-8: Case Detail E2E Tests', () => {
  ...
  test('Day 5: Login and navigate to case detail', async ({ page }) => {
    ...
    if (caseLinkCount > 0) {
      ...
      await expect(page.locator('#case-thread-container, [id*="thread"]')).toBeVisible({ timeout: 5000 }).catch(() => {});
      await expect(page.locator('#case-checklist-container, [id*="checklist"]')).toBeVisible({ timeout: 5000 }).catch(() => {});
      await expect(page.locator('#case-evidence-container, [id*="evidence"]')).toBeVisible({ timeout: 5000 }).catch(() => {});
    } else {
  ...
  test('Day 6: Post a message in thread', async ({ page }) => {
    ...
    if (caseLinkCount > 0) {
      ...
      // Find message input
      const messageInput = page.locator('input[name="body"], textarea[name="body"], input[placeholder*="message" i], textarea[placeholder*="message" i]').first();
      ...
      if (inputCount > 0) {
        ...
        await expect(page.locator('body')).toContainText(testMessage, { timeout: 3000 }).catch(() => {
  ...
  test('Day 7: Checklist displays steps', async ({ page }) => {
    ...
    if (caseLinkCount > 0) {
      ...
      await expect(checklistContainer).toBeVisible({ timeout: 5000 }).catch(() => {
  ...
  test('Day 8: Evidence cell loads', async ({ page }) => {
    ...
    if (caseLinkCount > 0) {
      ...
      await expect(evidenceContainer).toBeVisible({ timeout: 5000 }).catch(() => {
