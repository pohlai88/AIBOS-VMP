...
test.describe('SOA Reconciliation Workflow', () => {
  ...
  test('should select SOA line and show focus view', async ({ page }) => {
    ...
    await page.waitForSelector('#soa-focus .vmp-label-kicker:has-text("selected line")', { timeout: 5000 });
  ...
  test('should export SOA reconciliation pack', async ({ page }) => {
    ...
    const [response] = await Promise.all([
      page.waitForResponse(resp => resp.url().includes('/api/soa/') && resp.url().includes('/export')),
