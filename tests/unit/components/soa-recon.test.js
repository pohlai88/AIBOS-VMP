...
describe('SOA Reconciliation Components', () => {
  ...
  beforeEach(async () => {
    ...
    // Use setup helper to create test data
    testData = await setupServerTestData(supabase, {
      caseData: { case_type: 'soa' },
    });
  });

  afterEach(async () => {
    // Cleanup SOA-specific data first
  ...
  describe('SOA Items', () => {
    it('should create SOA item', async () => {
      const { data, error } = await supabase
        .from('vmp_soa_items')
        .insert({
          case_id: testData.case.id,
          vendor_id: testData.vendor.id,
          company_id: testData.case.company_id,
  ...
  describe('Debit Notes', () => {
    it('should create debit note proposal', async () => {
      ...
      const { data, error } = await supabase
        ...
        .insert({
          vendor_id: testData.vendor.id,
          soa_statement_id: testData.case.id,
